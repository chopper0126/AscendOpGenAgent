...
description: “初始化 AscendC 算子工程并创建可编译的算子骨架。严格触发条件：用户必须明确使用”初始化算子工程”、”生成算子框架”、”搭算子骨架”、”创建算子工程结构”等表述时才调用。禁止在一般性算子实现请求、性能优化讨论、算子转译、算子验证等场景中自动触发。本 skill 只负责搭建目录和标准文件骨架，不涉及具体算子逻辑实现。”
---
...
| 场景 | 用户表述示例 |
|------|-------------|
| 明确初始化 | "初始化算子工程"、"init operator project" |
| 生成框架/骨架 | "生成算子框架"、"搭算子骨架"、"创建算子工程结构" |
| 从零搭建 | "新建 AscendC 算子工程"、"给我搭一个算子开发的架子" |

**以下场景禁止自动触发**：
- 一般性算子实现请求（如"帮我实现一个 ELU 算子"）→ 应使用 `tilelang-designer` / `ascendc-translator` 等流程
- 性能优化讨论（如"这个算子怎么优化"）→ 应使用 `latency-optimizer`
- 算子验证请求（如"验证这个算子对不对"）→ 应使用 `kernel-verifier`
- 已有算子工程的修改、迭代、转译 → 不应重复初始化
